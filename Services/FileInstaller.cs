...
public static class FileInstaller
{
    ...
    /// <summary>
    /// Best-effort LIFO deletion of previously-written files. Swallows per-file
    /// errors because the caller is already in an error path. Used by the
    /// extraction loop on failure and by <c>InstallQueue</c> when a post-extraction
    /// registration step fails and the on-disk files must be undone.
    /// </summary>
    public static Task RollbackAsync(IReadOnlyList<string> files)
    {
