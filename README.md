...
## Current Release: v3.6.1 — Stability and UI Maintenance

v3.6.1 is a maintenance release that hardens the installation pipeline and polishes the user interface.

### Key Changes:
- **Atomic Rollback Safety:** The installer now creates temporary backups of existing files before overwriting. If an install fails, original files are restored automatically.
- **WebView2 Robustness:** Guarded initialization and interaction with the embedded browser to prevent crashes if the runtime is missing or busy.
- **WPF UI Polish:** Refined button visual states (hover, pressed, disabled) and improved accessibility with Automation properties.
- **Refined Release Package:** Documentation and license are now bundled directly in the release ZIP.

**Download:** [LSPDFRManager-v3.6.1-win-x64.zip](https://github.com/rolling-codes/LSPDFRManager/releases/download/v3.6.1/LSPDFRManager-v3.6.1-win-x64.zip)

See the [v3.6.1 Release Notes](RELEASE_v3.6.1.md) for full details.
...
1. Download `LSPDFRManager-v3.6.0-win-x64.zip` from the [Releases](https://github.com/rolling-codes/LSPDFRManager/releases) page.
