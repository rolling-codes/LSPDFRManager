...
| Tab | What it does |
|-----|-------------|
| **Library** | Browse all installed mods, search by name/author/type, filter by category and enabled/disabled state, sort by install date/name/author/status, bulk enable/disable/toggle visible mods, uninstall |
| **Install** | Drag-and-drop or browse for a mod archive (`.zip` / `.rar` / `.7z`); auto-detects mod type with confidence scoring; runs pre-install environment checks (RPH/LSPDFR/ScriptHookV/path warnings); installs into your GTA V folder with safer archive path handling |
