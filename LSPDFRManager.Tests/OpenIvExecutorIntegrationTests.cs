...
public class OpenIvExecutorIntegrationTests : IDisposable
{
    ...
    [Fact]
    public async Task Executor_OverwritesExistingFile_RollsBackToOriginalVersion()
    {
        // 1. Setup an existing file in the target
        var destPath = Path.Combine(_tempRoot, @"mods\file_to_overwrite.dll");
        Directory.CreateDirectory(Path.GetDirectoryName(destPath)!);
        var originalContent = "ORIGINAL_VERSION";
        File.WriteAllText(destPath, originalContent);

        // 2. Setup a plan that overwrites this file but then fails on a second file
        var archive = FakeArchiveFactory.CreateMidStreamFailureArchive(failAfterBytes: 5);
        var plan = new OpenIvInstallPlan
        {
            Type = CarInstallType.ReplaceVehicle,
            TargetDlcName = "failing_overwrite_mod",
            Operations = new()
            {
                new() { SourcePath = "file1.dll", DestinationPath = @"mods\file_to_overwrite.dll", Overwrite = true },
                new() { SourcePath = "file2.dll", DestinationPath = @"mods\trigger_failure.dll", Overwrite = true }
            },
            XmlPatches = new()
        };
        ...
        // 4. Verify failure and RESTORATION
        Assert.False(result.Success);
        Assert.True(File.Exists(destPath), "Destination file should still exist");
        
        var currentContent = File.ReadAllText(destPath);
        Assert.Equal(originalContent, currentContent); // Should have been restored from backup
        
        Assert.False(File.Exists(Path.Combine(_tempRoot, @"mods\trigger_failure.dll")), "Failed file should not exist");
    }
