...
public class OpenIvExecutorIntegrationTests : IDisposable
{
    ...
    // ── Test 6: PathSafety Defense-in-Depth ──────────────────────────────

    [Fact]
    public async Task Executor_TraversalThroughModsPrefix_BlockedByPathSafety()
    {
        // The validator's StartsWith("mods\") check passes this path, but the
        // resolved fullpath escapes targetRoot. PathSafety must reject it.
        var archive = FakeArchiveFactory.CreateCleanArchive("car.yft");
        var plan = new OpenIvInstallPlan
        {
            Type = CarInstallType.ReplaceVehicle,
            TargetDlcName = "evil_mod",
            Operations = new()
            {
                new() { SourcePath = "car.yft", DestinationPath = @"mods\..\..\escape.yft", Overwrite = true }
            },
            XmlPatches = new()
        };

        OpenIvInstallPlanValidator.Validate(plan); // passes — StartsWith("mods\")
        ...
        // No file should have escaped the target root.
        var escapedFile = Path.GetFullPath(Path.Combine(_tempRoot, @"..\..\escape.yft"));
        Assert.False(File.Exists(escapedFile), $"File escaped targetRoot: {escapedFile}");
    }
