...
3. Add file to rollback list (BEFORE the copy starts)
4. Copy stream → file

The rollback ledger is appended before the copy so that partial writes
(File.Create succeeded, CopyToAsync threw mid-stream) are still cleaned
up by RollbackAsync. PathSafety failures occur in step 1, before the
ledger is touched, so a rejected entry never enters the rollback set.
