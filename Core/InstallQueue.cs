    ...
    private async Task ProcessLoop()
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                // Post-extraction registration is transactional with the on-disk files:
                // any failure here must roll back the extracted files and any partially-
                // applied library/dlclist state so the system never holds tracked-but-
                // missing or untracked-but-present mods.
                bool addedToLibrary = false;
                bool addedToDlcList = false;
                try
                {
                ...
                {
                    AppLogger.Error($"[POST_INSTALL_FAILED] {mod.Name} | rolling back files={result.WrittenFiles.Count} | library={addedToLibrary} | dlc={addedToDlcList}", ex);

                    if (addedToDlcList)
                        DlcListService.RemoveEntry(installed.DlcPackName);
                    if (addedToLibrary)
                        ModLibraryService.Instance.Remove(installed.Id);
                    await FileInstaller.RollbackAsync(result.WrittenFiles).ConfigureAwait(false);

                    var failResult = new InstallResult
                    {
                        Success = false,
                        IsPartial = result.WrittenFiles.Count > 0,
                        FilesWritten = result.WrittenFiles.Count,
                        Error = ex.Message,
                    };
                    InstallFailed?.Invoke(mod, ex.Message);
                    InstallFailedWithResult?.Invoke(mod, failResult);
                }
            }
            catch (Exception ex)
            {
