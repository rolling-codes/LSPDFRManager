...
public class OpenIvExecutor
{
    ...
    {
        var rollbackStack = new Stack<RollbackAction>();
        var backupDir = Path.Combine(Path.GetTempPath(), "LSPDFRManager_Backup_" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            foreach (var operation in plan.Operations)
            {
                ...
                {
                    backupPath = Path.Combine(backupDir, Guid.NewGuid().ToString("N"));
                    File.Copy(destPath, backupPath);
                    AppLogger.Info($"[BACKUP] {Path.GetFileName(destPath)} -> {Path.GetFileName(backupPath)}");
                }

                // Track before write — File.Create creates the file before any data lands,
                // so a mid-stream failure still produces a file that rollback must delete/restore.
                rollbackStack.Push(new RollbackAction(destPath, backupPath));
            ...
            {
                Success = true,
                FilesWritten = rollbackStack.Count
            };
        }
        catch (Exception ex)
        {
            int writtenCount = rollbackStack.Count;
            AppLogger.Error($"[PLAN_ERROR] written={writtenCount}", ex);
            await RollbackAsync(rollbackStack, ct);

            // Clean up backup dir after rollback
            if (Directory.Exists(backupDir))
                Directory.Delete(backupDir, true);
    ...
    private record RollbackAction(string DestinationPath, string? BackupPath);

    private static Task RollbackAsync(Stack<RollbackAction> actions, CancellationToken ct)
    {
        int rollbackCount = actions.Count;
        ...
        while (actions.Count > 0)
        {
            ...
            var file = action.DestinationPath;

            try
            {
                // Do not observe ct here — rollback must complete regardless of cancellation
                if (File.Exists(file))
                {
                    if (action.BackupPath != null && File.Exists(action.BackupPath))
                    {
                        File.Copy(action.BackupPath, file, true);
                        restoredCount++;
                        AppLogger.Info($"[ROLLBACK_RESTORE] {Path.GetFileName(file)}");
                    }
                    else
                    {
                        File.Delete(file);
